...
function AtlanticStandings() {
  const [records, setRecords] = useState(null);
    useEffect(() => {
        getData();

        async function getData() {
            const response = await fetch("https://statsapi.web.nhl.com/api/v1/standings/byLeague");
                ...
                <table className="standings-table">
                    <thead>
                        <tr>
                            <th></th>
                            <th></th>
                            <th className="title">National Hockey League</th>
                            <th>Pts</th>
                            <th>GP</th>
                            <th>W</th>
                            <th>L</th>
                    ...
                    <tbody>
                    {records['records'][0]['teamRecords'].map((record, index) => (
                        <tr key={index}>
                            <td>{index + 1}</td>
...
export default AtlanticStandings